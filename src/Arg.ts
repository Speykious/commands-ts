...
export class Arg<T> {
	...
	constructor(types: ArgTypeTuple<any[]>, info: ArgInfo) {
		...
		if (typeof info.type === 'string') {
			...
			if (/^(word|text|int|float)$/.test(info.type)) {
				if (info.oneOf) {
					...
					if (info.min || info.max)
						throw `min/max options are incompatible with oneOf`;
					valParser = valParser.filter(
						(fresult) => isOneOf(fresult, info.oneOf),
						(from) => {
							const recieved = from.targetString.slice(from.index);
							return {
								info: `Argument has to be one of the following values: ${info.oneOf
									.map((v) => (typeof v === 'string' ? `"${v}"` : v))
									.join(', ')}, recieved "${recieved}" instead`,
								oneOf: info.oneOf,
								input: recieved
							};
						}
			...
			if (info.min || info.max)
				throw `min/max options are incompatible with union types`;

			const filtered: ArgTypeTuple<any[]> = new ArgTypeTuple();
			for (let name of info.type) filtered.push(types.getType(name));

			this.type = {
				name: info.type.join('-'),
				description: filtered
					.map((argt) => `${argt.name} -> ${argt.description}`)
					.join('\n'),
		...
		if (info.error) valParser = valParser.mapError(info.error);
