...
test('Options: with one argument - answer', async () => {
	...
	expect(short1.error.info).toBe(
		'Argument n°1 from option "answer" is invalid'
	);
	expect(short2.error.info).toBe(
		'Argument n°1 from option "answer" is invalid'
	);
	expect(short3.error).toBe(null);
	expect(short4.error.info).toBe(
		'Argument n°1 from option "answer" is invalid'
	);
...
test('Options: with multiple arguments - answer', async () => {
	...
	const long_false1 = await quizz.fullParse(
		`--Quizz neither 2 Kuriminaru 9999`
	);
	...
	const long_false4 = await quizz.fullParse(
		`--Quizz false 2 Kuriminaru banana`
	);
